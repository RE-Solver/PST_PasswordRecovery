...
0x00000000, 0x77073096, 0xee0e612c, 0x990951ba,
    ...
    0xb40bbe37, 0xc30c8ea1, 0x5a05df1b, 0x2d02ef8d,
        ...
        private uint[] reverseCrc32Table =
            {
    ...
    0xdd, 0x9c, 0x1e, 0x5f, 0x5b, 0x1a, 0x98, 0xd9
        ...
        void append_block(void* buffer, uint size)
        {
            byte* ptr = (byte*)buffer;
            crc_ = ReverseBytes(crc_);
        ...
        char[] patchChars = new char[7];
        byte[] findReverse(uint desiredCrc)
        {

            uint[] crcIdx= new uint[4];

            uint iterCrc = ReverseBytes(desiredCrc);
            for (int j = 3; j >= 0; j--)
            {
            ...
            uint crc = ReverseBytes(crc_);

            for (int j = 0; j < 4; j++)
            {
                patchBytes[j] = (byte)((crc ^ crcIdx[j]) & 0xff);
        ...
        bool printableASCII(char a)
        { return (((a) >= '0' && (a) <= '9') || ((a) >= 'A' && (a) <= 'Z') || ((a) >= 'a' && (a) <= 'z')); }
        int toprintableASCII(int a)
        {
            return (a) < 10 ? (a) + '0' : (a) < 36 ? ((a) - 10 + 'A') : (a) - 36 + 'a';
        }
        public char[] findReverseAscii(uint desiredCrc)
        {
            ...
            for (int a = 0; a < 62; a++)
            {
                ...
                {
                    crc_ = 0;
                    append((byte)toprintableASCII(a));
                    append((byte)toprintableASCII(b));
                    byte[] reverse = findReverse(desiredCrc);

                    // If the reverse CRC has valid ASCII characters, we found a solution.
                    if ((printableASCII((char)reverse[0])) && (printableASCII((char)reverse[1])) &&
                        (printableASCII((char)reverse[2])) && (printableASCII((char)reverse[3])))
                    {
                        patchChars[0] = (char)toprintableASCII(a);
                        patchChars[1] = (char)toprintableASCII(b);
